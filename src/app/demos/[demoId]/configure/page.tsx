...
export default function DemoConfigurationPage({ params }: { params: { demoId: string } }) {
  ...
  const handleIndexWithTwelveLabs = async (videoId: string): Promise<{ success: boolean; error?: string }> => {
    if (process.env.NODE_ENV !== 'production') { console.log('[handleIndexWithTwelveLabs] Starting for video:', videoId);
 }    try {
      ...
      if (process.env.NODE_ENV !== 'production') { console.log('[handleIndexWithTwelveLabs] Response status:', response.status);
 }
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
      ...
      const data = await response.json();
      if (process.env.NODE_ENV !== 'production') { console.log('[handleIndexWithTwelveLabs] Response data:', data);
 }
      if (data.success) {
        if (process.env.NODE_ENV !== 'production') { console.log('[handleIndexWithTwelveLabs] Success! Updating video metadata');
