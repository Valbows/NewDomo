...
async function dailyRoomExists(url: string): Promise<boolean> {
  ...
  try {
    const resp = await fetch(
      `https://gs.daily.co/rooms/check/${parsed.domain}/${parsed.room}`
    );
    return resp.ok;
  } catch (_) {
    return false;
...
async function handlePOST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (!forceNew) {
      try {
        const md =
          typeof (demo as any).metadata === "string"
            ? JSON.parse((demo as any).metadata as string)
            : (demo as any).metadata;
        ...
        if (existingUrl && isDailyRoomUrl(existingUrl)) {
          if (await dailyRoomExists(existingUrl)) {
            ...
            console.warn(
              "Existing Daily URL appears stale or missing. Creating a new conversation:",
              existingUrl
            );
          }
        }
        ...
        console.warn(
          "Failed to parse demo metadata while checking for existing conversation URL:",
          e
        );
      }
    } else {
      console.log(
        "forceNew=true: will create a new conversation even if an existing URL is present."
      );
    }
    ...
    if (!forceNew && startLocks.has(demoId)) {
      console.log(
        "Conversation start already in progress for demo",
        demoId,
        "— waiting"
      );
      try {
      ...
      try {
        const md2 =
          typeof afterDemo?.metadata === "string"
            ? JSON.parse(afterDemo?.metadata as any)
            : (afterDemo as any)?.metadata;
    ...
    if (!finalReplicaId) {
      try {
        ...
        if (personaResp.ok) {
          ...
          if (finalReplicaId) {
            console.log(
              "Using persona default_replica_id for conversation:",
              finalReplicaId
            );
          } else {
            console.warn(
              "Persona has no default_replica_id; a replica_id must be provided via TAVUS_REPLICA_ID."
            );
          }
        } else {
          const t = await personaResp.text();
          console.warn(
            "Failed to fetch persona for default_replica_id; status:",
            personaResp.status,
            t
          );
        }
      ...
      console.log(
        "Using replica_id from env for conversation:",
        finalReplicaId
      );
    }
    ...
    const baseUrlForWebhook = (
      process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"
    ).replace(/\/$/, "");
    const urlToken = (process.env.TAVUS_WEBHOOK_TOKEN || "").trim();
    const callbackUrl = `${baseUrlForWebhook}/api/tavus-webhook${
      urlToken ? `?t=${encodeURIComponent(urlToken)}` : ""
    }`;
    ...
    if (!conversationPayload.replica_id) {
      const msg =
        "Missing replica_id: Set TAVUS_REPLICA_ID or assign a default replica to the persona.";
    ...
    const doStart = async (): Promise<NextResponse | any> => {
      ...
      if (!conversationResponse.ok) {
        const contentType =
          conversationResponse.headers.get("content-type") || "";
        let errorBody: any;
        try {
      ...
      const updatedMetadata = {
        ...(typeof currentDemo?.metadata === "string"
          ? (() => {
              try {
                return JSON.parse(currentDemo?.metadata as any);
              } catch {
                return {};
              }
            })()
      ...
      if (updateError) {
        logError(
          updateError,
          "Supabase update error after starting conversation"
        );
    ...
    if (startLocks.has(demoId)) {
      ...
      try {
        const md3 =
          typeof afterDemo2?.metadata === "string"
            ? JSON.parse(afterDemo2?.metadata as any)
            : (afterDemo2 as any)?.metadata;
