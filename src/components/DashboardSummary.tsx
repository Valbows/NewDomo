...
function getAnalyticsMeta(demo: Demo) {
  ...
  return {
    ...
    conversationCount: analytics?.conversations ? Object.keys(analytics.conversations).length : 0,
  };
}
...
const DashboardSummary: React.FC<Props> = ({ demos, loading = false }) => {
  const { totalDemos, activeDemos, totalConversations, lastUpdated } = aggregate(demos);

  return (
    <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4" aria-live="polite">
      <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
        ...
        {loading ? (
          ...
          <p className="text-2xl font-semibold text-domo-dark-text" data-testid="summary-total-demos">{totalDemos}</p>
        )}
      </div>
      <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
        ...
        {loading ? (
          ...
          <p className="text-2xl font-semibold text-domo-dark-text" data-testid="summary-active-demos">{activeDemos}</p>
        )}
      </div>
      <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
        ...
        {loading ? (
          ...
          <p className="text-2xl font-semibold text-domo-dark-text" data-testid="summary-total-conversations">{totalConversations}</p>
        )}
      </div>
      <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-100">
        ...
        {loading ? (
          ...
          <p className="text-sm font-medium text-domo-dark-text" data-testid="summary-last-updated">
