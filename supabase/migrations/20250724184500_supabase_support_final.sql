...
-- Final solution for persistent JSON metadata errors
...
USING 
  CASE 
    ...
    ELSE COALESCE(
      CASE 
        WHEN jsonb_typeof(metadata::jsonb) = 'object' 
        THEN metadata::jsonb 
        ELSE '{}'::jsonb 
      END, 
      '{}'::jsonb
...
-- Add simplified metadata validation to avoid constraint issues
DO $$ 
BEGIN
    ...
    -- Add simple metadata validation
    ALTER TABLE demos 
